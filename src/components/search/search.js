...
export default class Search extends Component {
  ...
  peredPeremSort = (id, stars) => {
    ...
    if (data.length > 0) {
      ...
      if (id) {
        ...
        if (!localStorage.getItem('dataStarsArray')) {
          ...
          //console.log(dataStarsArray)
          localStorage.setItem('dataStarsArray', JSON.stringify(dataStarsArray))
          //
          //
        } else {
          const dataStarsArray = JSON.parse(localStorage.getItem('dataStarsArray')) //нахожу массив выбраных значений
          //console.log(dataStarsArray)

          const findObject = dataStarsArray.find((el) => el.id === id) // нахожу элемент в массив по id
          if (findObject) {
            const newDataStarsArray = dataStarsArray.map((el) => (el.id === id ? { ...el, stars: stars } : el))
            //console.log(newDataStarsArray)
    ...
    if (proverka && proverka.find((el) => el.stars === 0)) {
      //исполнится если есть с 0 звезд
      const newDataStarsArray = proverka.filter((el) => el.stars !== 0) //фильтр получаем список элементов со звездами не 0
      //console.log(newDataStarsArray)
      localStorage.removeItem('dataStarsArray')
      //console.log(JSON.parse(localStorage.getItem('dataStarsArray')))
      localStorage.setItem('dataStarsArray', JSON.stringify(newDataStarsArray))
      //
      //
    }
  ...
  onMovieListLoaded = (movies) => {
    ...
    // const a = movies.filter((item) => item.average)
    // this.props.filteredData(a, this.state.value)
